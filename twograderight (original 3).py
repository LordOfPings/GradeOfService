			...
			if temp_value == 1:
				for j in range (int(channels_high),number_wavelengths):
					if channel_available[j] == 1:
						...
						backup_channels_all[request_key] = j #op.write(str(j) + " is the backup channel allocated\n")
						break
				if tmp == 0:
					return 1
				else:
    ...
    if request[2] > 0.5*number_wavelengths:
                	...
                	if temp2 == 1:
            			...
            			for j in range (int(channels_high)):
            				if channel_available[j] == 1:
	            				...
	            				primary_channels_high_all[request_key] = j
	            				established_high += 1
            					break
	                else:
	                	#path = []
        ...
        while True:
            ...
            if len(rev_path) == 0:
                ...
                #continue # if no path is found in the first place so request is blockes
            else:
                path = []
                path.append(request[0])
                for i in range(len(rev_path) - 1, -1, -1):
                ...
                if temp == 1:
            		for j in range (int(channels_high),number_wavelengths):
            			if channel_available[j] == 1 and int(dist[tgt]) < 9999:
	            			...
	            			primary_channels_low_all[request_key] = j
	            			established_low += 1
            				break
            		if tmp == 1:
            			print("no1 primary was path found; request blocked")
                        primary_paths_low_all[request_key] = "Blockeed"
                        break
                else:
                    if dist[tgt] < 9999:
                        continue
...
for key in primary_paths_low_all.values():
	if key == "Blocked":
		number_blocked_requests += 1
getcontext().prec = 6
#twograde_output(op,number_requests,number_blocked_requests,number_low,number_high,primary_paths_high_all,primary_paths_low_all,backup_paths_all,established_low,established_high)
...
print("Number of LightPaths Established (NLE): " + str(number_blocked_requests))
